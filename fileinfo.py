...
#最开始需要运行的函数以及初始界面
text_edit(original_text,stop_words)
all_number(text_edited)
most_num(num_word,num_num)
top=Tk()
top.title('文本编辑器')
button_print_origin=Button(top,text='打印原文',command=lambda:(print_text(original_text)))
button_print_len=Button(top,text='文章总字数',command=lambda:(print_total_num(original_text)))
button_num=Button(top,text='词数统计',command=lambda:(count_word(num_word,num_num)))
button_exit=Button(top,text='退出',command=destroy)
button_print_origin.grid(row=0,column=0)
button_print_len.grid(row=1,column=0)
